...
s32_sprite #(.POST_VBLANK_CYCLES(8)) sprite (
    .clk(clk), .rst(rst), .is_multi32(MULTI32), .screen_sel(1'b0),
    .present(present),
    .verify_srom(1'b0),
...
always @(posedge clk) begin
    ...
    if (!rst && (fbe_req || fbw_start || fbw_valid)) begin
        active_buf = fbe_req ? fbe_buf : fbw_buf;
        active_scan = {active_buf[1], scan_buf[0]};
        if ((MULTI32 && (active_buf == active_scan)) ||
            (!MULTI32 && (active_buf == scan_buf))) begin
            errors = errors + 1;
...
initial begin
    ...
    if (MULTI32) begin
        // One 4-bpp 1:1 sprite followed by END. This exercises write ownership
        // after both physical monitor banks have been erased.
        sram[0] = 16'h000a;
        sram[1] = 16'h0102;
        sram[2] = 16'h0001;
        sram[3] = 16'h0008;
        ...
        sram[7] = 16'h0100;
        sram[8] = 16'hc000;
        srom_data = 128'h00000000000000000000000078563412;
    end
    ...
    if (errors == 0 && MULTI32) $display("SPRITE MULTI32 OWNERSHIP PASS");
    else if (errors == 0) $display("SPRITE VBLANK PASS");
