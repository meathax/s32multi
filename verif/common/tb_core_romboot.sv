...
wire [15:0] fbw_pix;
wire        fbw_shadow;
wire        fbw_busy, fbw_can_start, fbe_ack, fbr_ack;
wire [15:0] fbr_pix;
...
wire [31:0] fb_ddr_writes, fb_sprite_ddr_writes, fb_run_starts, fb_run_pixels;
wire [31:0] fb_ddr_reads, fb_line_acks, fb_line_acks_a, fb_line_acks_b;
wire [31:0] fb_max_wr_wait, fb_max_rd_wait, fb_max_rd_wait_a, fb_max_rd_wait_b;
wire [31:0] fb_max_er_wait;
wire        fb_deadline_fail;
wire        fb_sprite_write_accepted;
...
s32_fb_ddr_model fb_service (
    ...
    .write_accepts(fb_ddr_writes),
    .sprite_write_accepts(fb_sprite_ddr_writes),
    .sprite_write_accepted(fb_sprite_write_accepted),
    .run_starts(fb_run_starts), .run_pixels(fb_run_pixels),
    .read_accepts(fb_ddr_reads), .line_acks(fb_line_acks),
    .line_acks_a(fb_line_acks_a), .line_acks_b(fb_line_acks_b),
    .max_wr_wait(fb_max_wr_wait), .max_rd_wait(fb_max_rd_wait),
    .max_rd_wait_a(fb_max_rd_wait_a), .max_rd_wait_b(fb_max_rd_wait_b),
...
`ifdef S32_REAL_FB_SIM
reg fb_title_check;
integer fb_title_runs = 0, fb_title_pixels = 0, fb_title_ddr_writes = 0;
integer fb_title_acks_a = 0, fb_title_acks_b = 0;
reg fb_title_ack_a_d = 0, fb_title_ack_b_d = 0;
initial fb_title_check = $test$plusargs("FBTITLE");
`endif
...
`ifdef S32_REAL_FB_SIM
    fb_title_ack_a_d <= fbr_ack;
    fb_title_ack_b_d <= fbr2_ack;
    if (cur_frame >= 296 && cur_frame <= 303) begin
        if (fbw_start) fb_title_runs = fb_title_runs + 1;
        if (fbw_valid) fb_title_pixels = fb_title_pixels + 1;
        if (fb_sprite_write_accepted)
            fb_title_ddr_writes = fb_title_ddr_writes + 1;
        if (fbr_ack && !fb_title_ack_a_d)
            fb_title_acks_a = fb_title_acks_a + 1;
        if (fbr2_ack && !fb_title_ack_b_d)
            fb_title_acks_b = fb_title_acks_b + 1;
    end
`endif
end
...
`ifdef S32_REAL_FB_SIM
    ...
    if (fb_line_acks < frames * 128)
        $fatal(1, "GA2 framebuffer line service too sparse: acks=%0d frames=%0d",
               fb_line_acks, frames);
    if (fb_line_acks_a == 0 || fb_line_acks_b == 0)
        $fatal(1, "Dual-screen framebuffer lane missing: A=%0d B=%0d",
               fb_line_acks_a, fb_line_acks_b);
    if (fb_title_check) begin
        if (frames < 304)
            $fatal(1, "OutRunners title DDR check stopped before frame 304: frames=%0d", frames);
        if (fb_title_runs == 0 || fb_title_pixels == 0 || fb_title_ddr_writes == 0)
            $fatal(1, "OutRunners title sprite traffic missing: runs=%0d pixels=%0d ddr_writes=%0d",
                   fb_title_runs, fb_title_pixels, fb_title_ddr_writes);
        if (fb_title_acks_a < 1000 || fb_title_acks_b < 1000)
            $fatal(1, "OutRunners title line service sparse: A=%0d B=%0d",
                   fb_title_acks_a, fb_title_acks_b);
        $display("OUTRUNNERS TITLE DDR PASS runs=%0d pixels=%0d ddr_writes=%0d acks_a=%0d acks_b=%0d max_rd_a=%0d max_rd_b=%0d",
            fb_title_runs, fb_title_pixels, fb_title_ddr_writes,
            fb_title_acks_a, fb_title_acks_b, fb_max_rd_wait_a, fb_max_rd_wait_b);
    end
    ...
    $display("FRAMEBUFFER DDR QUALIFICATION PASS writes=%0d sprite_writes=%0d runs=%0d pixels=%0d reads=%0d line_acks=%0d (A=%0d B=%0d) max_wr=%0d max_rd=%0d (A=%0d B=%0d) max_er=%0d sig_samples=%0d sig_changes=%0d",
        fb_ddr_writes, fb_sprite_ddr_writes, fb_run_starts, fb_run_pixels,
        fb_ddr_reads, fb_line_acks, fb_line_acks_a, fb_line_acks_b,
        fb_max_wr_wait, fb_max_rd_wait, fb_max_rd_wait_a, fb_max_rd_wait_b, fb_max_er_wait,
        frame_sig_samples, frame_sig_changes);
`endif
    if (eepdbg) begin
