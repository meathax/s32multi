...
iverilog -g2012 -DSIMULATION -o /tmp/s32_lint \
  ...
  rtl/video/*.sv rtl/audio/s32_multipcm.sv \
...
iverilog -g2012 -o /tmp/s32_mpcm_level rtl/audio/s32_multipcm.sv verif/common/tb_multipcm_output_level.sv
vvp /tmp/s32_mpcm_level | grep -q "AUDIO_LEVEL_RATIO reference_over_rtl=1" \
  && echo "MULTIPCM OUTPUT LEVEL: PASS" || { echo "MULTIPCM OUTPUT LEVEL: FAIL"; exit 1; }
iverilog -g2012 -o /tmp/s32_mpcm_pan rtl/audio/s32_multipcm.sv verif/common/tb_multipcm_pan.sv
vvp /tmp/s32_mpcm_pan | grep -q "MULTIPCM PAN PASS cases=32" \
  && echo "MULTIPCM PAN LAW: PASS" || { echo "MULTIPCM PAN LAW: FAIL"; exit 1; }
