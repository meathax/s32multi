...
class GlobalProfileContractTests(unittest.TestCase):
    ...
    def test_multi32_service_controls_are_local_to_each_screen(self) -> None:
        """Each Multi 32 315-5296 service port sees only its own cockpit.

        The OSD Service Mode bit is intentionally global, but physical Test
        and Service buttons must not be ORed between the two screens. This is
        the source-level guard for the independent SERVICE12/SERVICE34 wiring
        used by the OutRunners dual-screen board.
        """
        text = (ROOT / "Arcade-SegaSystem32Multi.sv").read_text(encoding="utf-8")
        self.assertIn("wire test_btn_a = status[7] | joystick_0[13];", text)
        self.assertIn("wire svc_btn_a  = joystick_0[14];", text)
        self.assertIn("wire test_btn_b = status[7] | joystick_1[13];", text)
        self.assertIn("wire svc_btn_b  = joystick_1[14];", text)
        self.assertIn("test_btn_a, svc_btn_a};", text)
        self.assertIn("test_btn_b, svc_btn_b};", text)
        self.assertNotIn("joystick_0[13] | joystick_1[13]", text)
        self.assertNotIn("joystick_0[14] | joystick_1[14]", text)

    def test_monitor_defaults_and_audio_route_are_explicit(self) -> None:
        """Zero-valued menu defaults select A/non-split, while the output
        router keeps split A/B stereo and duplicates only the selected monitor
        when displaying a single screen."""
        text = (ROOT / "Arcade-SegaSystem32Multi.sv").read_text(encoding="utf-8")
        self.assertIn('"O[6],Screen (Multi32),A,B;"', text)
        self.assertIn('"O[38],Splitscreen,Off,On;"', text)
        self.assertIn("wire [23:0] game_rgb = status[6] ? rgb_b : rgb_a;", text)
        self.assertIn("wire splitscreen_en = status[38];", text)
        self.assertIn("s32_monitor_audio_route monitor_audio_route", text)
        self.assertIn(".screen_sel(status[6]), .splitscreen(splitscreen_en)", text)
    ...
    def test_production_video_path_strips_crt_adjust(self) -> None:
        text = (ROOT / "Arcade-SegaSystem32Multi.sv").read_text(encoding="utf-8")
        qsf = (ROOT / "Arcade-SegaSystem32Multi.qsf").read_text(encoding="utf-8")
        ...
        self.assertNotIn("CRT Adjust", text)
        self.assertNotIn("crt_adjust", text)
        self.assertNotIn("rtl/crt_adjust.sv", qsf)
        self.assertNotIn("rtl/crt_adjust.sv", regression)
        self.assertNotIn("rtl/crt_adjust.sv", shell_regression)
        self.assertIn("assign CE_PIXEL = disp_ce_pix;", text)
        self.assertIn("assign VGA_HS = disp_hs;", text)
        self.assertIn("assign VGA_VS = disp_vs;", text)
    ...
    def test_driving_wheel_sources_reach_both_adc_channels(self) -> None:
        text = (ROOT / "Arcade-SegaSystem32Multi.sv").read_text(encoding="utf-8")
        controls = (ROOT / "rtl/io/s32_driving_controls.sv").read_text(
            encoding="utf-8")
        self.assertIn(".paddle_0(paddle_0)", text)
        self.assertIn(".paddle_1(paddle_1)", text)
        self.assertIn(".spinner_0(spinner_0)", text)
        self.assertIn(".spinner_1(spinner_1)", text)
        self.assertIn(
        ...
        self.assertIn(".paddle(paddle_0)", text)
        self.assertIn(".spinner(spinner_0)", text)
        self.assertIn(".wheel_source(status[40:39])", text)
        self.assertIn(".left_x(joystick_l_analog_1[7:0])", text)
        self.assertIn(".paddle(paddle_1)", text)
        self.assertIn(".spinner(spinner_1)", text)
        self.assertIn(".wheel_source(status[42:41])", text)
        self.assertIn(".right_y(joystick_r_analog_0[15:8])", text)
        self.assertIn("assign adc_ch[0] = driving_wheel;", text)
        self.assertIn("assign adc_ch[3] = driving_wheel_p2;", text)
        self.assertNotIn("gun_aim ? gun_adc", text)
        self.assertIn("wheel_pending_valid", controls)
        self.assertIn("wheel_sample", controls)
        self.assertIn("spinner_position", controls)
        self.assertIn("spinner[8] != spinner_toggle_d", controls)
        self.assertIn("2'd1: wheel_source_value = paddle", controls)
